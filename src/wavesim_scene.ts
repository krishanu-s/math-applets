...
(function () {
  document.addEventListener("DOMContentLoaded", async function () {
    ...
    await (async function twodim_dipole_demo() {
      ...
      sim.set_pml_layer(false, false, 0.2, 200.0);
    ...
    await (async function wave_sim_2d_doubleslit(
      ...
      for (let x = 0; x < width; x++) {
        sim.add_point_source(x, 0, w, a, 0.0);
      }
      ...
      function make_apertures(
        ...
        let sources: Array<[number, number, number, number, number]> = [];
        for (let x = 0; x < width; x++) {
          if (Math.abs(x / width - (0.5 - slit_dist / 2)) < slit_width) {
          } else if (Math.abs(x / width - (0.5 + slit_dist / 2)) < slit_width) {
          } else {
            sources.push([x, Math.floor(height * wall_height), 0, 0, 0]);
          }
        }
        return sources;
      }
