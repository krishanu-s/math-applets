...
export class MathJaxRenderer {
   ...
   */
  static async init(config: MathJaxConfig = {}): Promise<void> {
    if (this.initialized) {
      return;
    }
   ...
   */
  private static async loadMathJax(config: MathJaxConfig): Promise<void> {
  ...
  static async latexToSVG(
    ...
    try {
      ...
      if (!svgElement) {
        ...
        throw new Error(
          `No SVG element found. Generated content: ${content.substring(0, 200)}...`,
        );
      }
  ...
  static async renderToCanvas(
    ...
    try {
      ...
      console.error(
        "Error in renderToCanvas, falling back to DOM method:",
        error,
      );
   ...
   */
  private static extractPathsFromSVG(svgElement: SVGElement): string[] {
    const paths: string[] = [];
   ...
   */
  static async typeset(elements: Element | Element[]): Promise<void> {
    await this.init();
   ...
   */
  static async typesetDocument(): Promise<void> {
    await this.init();
...
export async function renderLatexToCanvas(
  canvas: HTMLCanvasElement,
  latex: string,
  x: number = 0,
  y: number = 0,
  ...
  return MathJaxRenderer.renderToCanvas(
    canvas,
    latex,
    x,
    y,
...
export async function latexToSVGPaths(
  latex: string,
  displayMode: boolean = false,
): Promise<string[]> {
