...
export class BezierSpline3D extends ThreeDMObject {
  ...
  _draw(ctx: CanvasRenderingContext2D, scene: ThreeDScene) {
    ...
    let a_x: number, a_y: number, a: Vec3D;
    a = this.get_anchor(0);
    [a_x, a_y] = scene.v2c(scene.camera_view(a) as Vec2D);
    ctx.beginPath();
    ctx.moveTo(a_x, a_y);
    ...
    try {
      ...
      for (let i = 0; i < this.num_steps; i++) {
        [h1_x, h1_y] = scene.v2c(scene.camera_view(handles[i]) as Vec2D);
        [h2_x, h2_y] = scene.v2c(
          scene.camera_view(handles[i + this.num_steps]) as Vec2D,
        );
