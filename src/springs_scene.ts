  ...
  document.addEventListener("DOMContentLoaded", async function () {
    ...
    (function point_mass_pendulum(width: number, height: number) {
      ...
      class PendulumScene extends SceneFromSimulator {
        ...
        // TODO Add callback
        add_callbacks() {
          let mass = this.get_mobj("mass") as DraggableDot;
          mass.add_callback(() => {
            let [x, y] = mass.get_center();
            let xdiff = x;
            let ydiff = y - (ymax + 0.2 * (ymin - ymax));
            let theta = Math.atan2(xdiff, -ydiff);
            sim.set_vals([theta, 0]);
            this.update_mobjects_from_simulator(sim);
          });
        }
        set_spring_stiffness(val: number) {
        ...
        // Specific to this scene and simulator
        _update_mass(vals: Vec2D) {
          let theta = vals[0] as number;
          let pos: Vec2D = [
            Math.sin(theta) * this.string_length,
            ymax + 0.2 * (ymin - ymax) - Math.cos(theta) * this.string_length,
          ];
          (this.get_mobj("mass") as Rectangle).move_to(pos);
        }
        _update_string(vals: Vec2D) {
          ...
          (this.get_mobj("string") as Line).move_end(pos);
        }
        _update_force_arrow(vals: Vec2D) {
          let force_arrow = this.get_mobj("force_arrow") as Arrow;
          let theta = vals[0] as number;
          ...
          force_arrow.move_start(pos);
          force_arrow.move_end([
            pos[0] -
              this.arrow_length_scale * Math.sin(theta) * Math.cos(theta),
            pos[1] -
              this.arrow_length_scale * Math.sin(theta) * Math.sin(theta),
          ]);
          force_arrow.set_arrow_size(
            Math.min(0.5, Math.sqrt(Math.abs(Math.sin(theta))) / 2),
          );
        }
        // Enforce strict order on drawing mobjects, overriding subclass behavior
        _draw() {
          this.draw_mobject(this.get_mobj("eq_line") as Line);
          this.draw_mobject(this.get_mobj("anchor") as Rectangle);
          this.draw_mobject(this.get_mobj("string") as Line);
          this.draw_mobject(this.get_mobj("mass") as Dot);
          this.draw_mobject(this.get_mobj("force_arrow") as Arrow);
        }
        draw_mobject(mobj: MObject) {
          mobj.draw(this.canvas, this);
      ...
      class Graph2Scene extends SceneFromSimulator {
        ...
        constructor(canvas: HTMLCanvasElement) {
          ...
          let axes = new CoordinateAxes2d(
            ...
            .set_axis_options({ stroke_width: 0.04, arrow_size: 0.1 })
            .set_tick_options({ size: 0.1 });
          ...
          this.add(
            "dot",
            new Dot(
              [sim.get_vals()[0] as number, sim.get_vals()[1] as number],
              0.08,
            ).set_color("black"),
          );
          this.draw();
        ...
        update_mobjects_from_simulator(simulator: DoubleSpringSim) {
          ...
          if (this.step_counter % 5 === 0) {
            (
              this.get_mobj(`graph${this.num_graphs - 1}`) as LineSequence
            ).add_point([vals[0] as number, vals[1] as number]);
            (this.get_mobj("dot") as Dot).move_to([
              vals[0] as number,
              vals[1] as number,
            ]);
          }
