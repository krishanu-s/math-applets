...
// node_modules/numpy-ts/dist/numpy-ts.browser.js
var np = (() => {
  ...
  var Er = { bool: 0, uint8: 1, int8: 2, uint16: 3, int16: 4, uint32: 5, int32: 6, uint64: 7, int64: 8, float32: 9, float64: 10, complex64: 11, complex128: 12 }, sr = { bool: "b", uint8: "u", uint16: "u", uint32: "u", uint64: "u", int8: "i", int16: "i", int32: "i", int64: "i", float32: "f", float64: "f", complex64: "c", complex128: "c" };
  ...
  var gt = new Uint8Array([147, 78, 85, 77, 80, 89]), kt = ["float64", "float32", "complex128", "complex64", "int64", "int32", "int16", "int8", "uint64", "uint32", "uint16", "uint8", "bool"];
  function En() {
    let r = new ArrayBuffer(2);
    return new DataView(r).setInt16(0, 256, true), new Int16Array(r)[0] === 256;
  }
  var qd = { f8: "float64", f4: "float32", c16: "complex128", c8: "complex64", i8: "int64", i4: "int32", i2: "int16", i1: "int8", u8: "uint64", u4: "uint32", u2: "uint16", u1: "uint8", b1: "bool" }, At = { float64: "<f8", float32: "<f4", complex128: "<c16", complex64: "<c8", int64: "<i8", int32: "<i4", int16: "<i2", int8: "|i1", uint64: "<u8", uint32: "<u4", uint16: "<u2", uint8: "|u1", bool: "|b1" }, gp = { S: "byte strings", U: "Unicode strings", O: "Python objects", V: "structured arrays (void)", M: "datetime64", m: "timedelta64" };
  function Ap(r) {
...
`;
  ...
  var br = 624, Lt = 397, ng = 2567483615, qn = 2147483648, Vn = 2147483647, hr = { mt: new Uint32Array(br), mti: br + 1 };
  function Bp(r) {
    let t = hr.mt;
    t[0] = r >>> 0;
    for (let e = 1; e < br; e++) {
  ...
  var og = 2468251765, ag = 1492356589, sg = 1135663077, ig = 2337405405, ug = 3389127133, cg = 1232336661, Wn = 16, Gt = 4;
  function ur(r) {
    return r >>> 0;
  }
  function jn(r, t) {
  ...
  var pg = BigInt("4865540595714422341"), yg = BigInt("2549297995355413924"), dg = yg << BigInt(64) | pg, Pn = BigInt("0xffffffffffffffff"), Ln = (BigInt(1) << BigInt(128)) - BigInt(1);
  function gg(r) {
...
var SmoothOpenPathBezierHandleCalculator = class {
  constructor(n) {
    ...
    let below_diag = (void 0)(below_diag_list);
    let diag_list = [2];
    for (let i = 0; i < n - 2; i++) {
      diag_list.push(4);
    }
    diag_list.push(7);
    let diag = (void 0)(diag_list);
    let above_diag_list = [];
    for (let i = 0; i < n - 1; i++) {
      above_diag_list.push(1);
    }
    let above_diag = (void 0)(above_diag_list);
    this.result = (void 0)([n, n + 1], "float32");
    ...
    for (let i = 0; i < n - 1; i++) {
      let scale = below_diag.get([i]) / diag.get([i]);
      diag.set(
        [i + 1],
        diag.get([i + 1]) - above_diag.get([i]) * scale
      );
      below_diag.set(
        [i],
        below_diag.get([i]) - diag.get([i]) * scale
      );
      for (let j = 0; j < n + 1; j++) {
        this.result.set(
          [i + 1, j],
    ...
    for (let i = n - 2; i >= 0; i--) {
      let scale = above_diag.get([i]) / diag.get([i + 1]);
      for (let j = 0; j < n + 1; j++) {
        this.result.set(
          [i, j],
...
var ParametricFunction = class extends LineLikeMObject {
  ...
  _draw(ctx, scene) {
    let points = [(void 0)(this.function(this.tmin))];
...
var WaveSimOneDimScene = class extends SceneFromSimulator {
  ...
  toggle_pause() {
    for (let i = 0; i < this.width; i++) {
      let dot2 = this.get_mobj(`p_${i + 1}`);
      dot2.draggable_y = true;
    }
  }
  toggle_unpause() {
    for (let i = 0; i < this.width; i++) {
      let dot2 = this.get_mobj(`p_${i + 1}`);
      dot2.draggable_y = false;
    }
  }
  ...
  update_mobjects_from_simulator(sim) {
    ...
    if (this.mode == "dots") {
      ...
      for (let i = 0; i < this.width; i++) {
        pos = this.eq_position(i + 1);
        disp = u[i];
        dot2 = this.get_mobj(`p_${i + 1}`);
        dot2.move_to([pos[0], pos[1] + disp]);
...
var WaveSimTwoDimPointsHeatmapScene = class extends SceneFromSimulator {
  ...
  construct_scene() {
    ...
    for (let i = 0; i < this.width; i++) {
      for (let j = 0; j < this.height; j++) {
        let dot2 = new Dot(this.eq_position(i, j), 0.15);
        this.add(`p(${i},${j})`, dot2);
      }
    }
  }
  ...
  update_mobjects_from_simulator(sim) {
    ...
    for (let i = 0; i < this.width; i++) {
      for (let j = 0; j < this.height; j++) {
        ...
        dot2 = this.get_mobj(`p(${i},${j})`);
        dot2.set_color(new_color2);
      }
    }
  }
};
...
var WaveSimTwoDimThreeDScene = class extends InteractivePlayingThreeDScene {
  ...
  construct_scene() {
    ...
    for (let i = 0; i < this.width; i++) {
      for (let j = 0; j < this.height; j++) {
        let dot2 = new DraggableDot3D(this.eq_position(i, j), 0.1);
        dot2.draggable_x = false;
        dot2.draggable_y = false;
        dot2.draggable_z = true;
        this.add_callbacks(i, j, dot2);
        this.add(`p(${i},${j})`, dot2);
      }
    }
  }
  ...
  toggle_pause() {
    if (this.paused) {
      for (let i = 0; i < this.width; i++) {
        for (let j = 0; j < this.height; j++) {
          let dot2 = this.get_mobj(`p(${i},${j})`);
          dot2.draggable_z = false;
        }
      }
    } else {
      for (let i = 0; i < this.width; i++) {
  ...
  update_mobjects() {
    ...
    for (let i = 0; i < this.width; i++) {
      for (let j = 0; j < this.height; j++) {
        ...
        dot2 = this.get_mobj(`p(${i},${j})`);
        [x, y, z] = dot2.get_center();
        dot2.move_to([x, y, new_z]);
        if (i < this.width - 1) {
