import { clamp, Dot, Line, LineSpring, Arrow, DraggableDot, vec2_norm, vec2_sub, rb_colormap_2, colorval_to_rgba, } from "../base";
import { BezierSpline } from "../base/bezier.js";
import { HeatMap } from "../base/heatmap";
import { SceneFromSimulator } from "../simulator/sim";
import { StateSimulator, TwoDimState } from "./statesim.js";
import { Line3D, DraggableDot3D } from "../three_d/mobjects.js";
...
export class WaveSimOneDim extends StateSimulator {
    constructor(width, dt) {
        ...
        this.pml_layers = {}; //
        this.wave_propagation_speed = 20.0; // Speed of wave propagation
        this.damping = 0.0; // Damping coefficient
        this.left_endpoint = 0.0; // Left boundary condition
        this.right_endpoint = 0.0; // Left boundary condition
        this.point_sources = {};
        this.width = width;
    }
    remove_pml_layers() {
    ...
    sigma_x(arr_x) {
        ...
        if (this.pml_layers.hasOwnProperty(ind)) {
            ...
            let relative_distance_from_center = Math.abs(-1 + arr_x / (this.width / 2));
            return (pml_strength *
    ...
    laplacian_entry(vals, x) {
        if (x == 0) {
            ...
            return (2 * this.laplacian_entry(vals, this.width - 2) -
    ...
    dot(vals, time) {
        ...
        for (let x = 0; x < this.width; x++) {
            dS.push(this.wave_propagation_speed ** 2 * this.laplacian_entry(u, x) -
                this.damping_at(x) * vals[x + this.width]);
        }
        return dS;
    }
...
export class WaveSimOneDimScene extends SceneFromSimulator {
    constructor(canvas, width) {
        ...
        for (let i = 1; i < width; i++) {
            ...
            let line = new LineSpring(pos, next_pos).set_stroke_width(0.2 / Math.sqrt(width));
        ...
        for (let i = 1; i < width - 1; i++) {
            ...
            let arrow = new Arrow([pos[0], pos[1]], [pos[0], pos[1]]).set_stroke_width(0.05);
            ...
            for (let i = 0; i < this.width; i++) {
                ...
                if (i != 0 && i != this.width - 1 && this.include_arrows) {
                    ...
                    arrow.move_end([
                        ...
                        pos[1] +
                            disp +
                            (this.arrow_length_scale * deriv[i]) / 5,
                    ]);
                    arrow.set_arrow_size(Math.sqrt(this.arrow_length_scale * Math.abs(deriv[i])) /
                        10);
                }
...
export class WaveSimTwoDim extends StateSimulator {
    constructor(width, height, dt) {
        super(4 * width * height, dt);
        this.pml_layers = {};
        this.wave_propagation_speed = 10.0; // Speed of wave propagation
        this.point_sources = {};
    ...
    sigma_y(arr_y) {
        ...
        if (this.pml_layers.hasOwnProperty(ind)) {
            [pml_thickness, pml_strength] = this.pml_layers[ind];
            let relative_distance_from_center = Math.abs(-1 + arr_y / (this.height / 2));
            return (pml_strength *
                Math.max(0, relative_distance_from_center + pml_thickness - 1) ** 2);
        }
        else {
                ...
                dS[ind + this.size()] =
                    this.wps(x, y) ** 2 * this.laplacian_entry(u, x, y) +
                        ...
                        (this.sigma_x(x) + this.sigma_y(y)) *
                            vals[ind + this.size()] -
                        this.sigma_x(x) * this.sigma_y(y) * vals[ind];
            }
        }
        // px dot
        for (let x = 0; x < this.width; x++) {
    ...
    set_boundary_conditions(s, t) {
        ...
        for (let ind = 0; ind < this.state_size; ind++) {
            this.vals[ind] = clamp(this.vals[ind], -this.clamp_value, this.clamp_value);
        }
    }
}
export class WaveSimTwoDimReflector extends WaveSimTwoDim {
    constructor(width, height, dt) {
    ...
    _recalculate_masks() {
        ...
        for (let y_arr = 0; y_arr < this.height; y_arr++) {
            for (let x_arr = 0; x_arr < this.width; x_arr++) {
                ...
                [this.x_pos_mask[ind], this.x_neg_mask[ind]] = this._calc_bdy_dists_x(x_arr, y_arr);
                [this.y_pos_mask[ind], this.y_neg_mask[ind]] = this._calc_bdy_dists_y(x_arr, y_arr);
            }
        }
    }
    ...
    d_x_entry(arr, x, y) {
        ...
        if (a_plus == 0 && a_minus == 0) {
            ...
            return (((a_minus * this._two_dim_state.d_x_plus(arr, x, y)) / a_plus +
    ...
    d_y_entry(arr, x, y) {
        ...
        if (a_plus == 0 && a_minus == 0) {
            ...
            return (((a_minus * this._two_dim_state.d_y_plus(arr, x, y)) / a_plus +
    ...
    l_x_entry(arr, x, y) {
        ...
        if (a_plus == 0 && a_minus == 0) {
            ...
            return ((this._two_dim_state.d_x_plus(arr, x, y) / a_plus -
    ...
    l_y_entry(arr, x, y) {
        ...
        if (a_plus == 0 && a_minus == 0) {
            ...
            return ((this._two_dim_state.d_y_plus(arr, x, y) / a_plus -
...
export class WaveSimTwoDimEllipticReflector extends WaveSimTwoDimReflector {
    constructor(width, height, dt) {
        ...
        this.semimajor_axis = 80.0;
        this.semiminor_axis = 60.0;
        this.w = 5.0; // Frequency
        this.a = 5.0; // Amplitude
        this.foci = [
            [
    ...
    inside_region(x_arr, y_arr) {
        return (((x_arr - this.width / 2) / this.semimajor_axis) ** 2 +
            ((y_arr - this.height / 2) / this.semiminor_axis) ** 2 <
            1);
    }
    _x_plus(x, y) {
        return Math.abs(this.semimajor_axis *
    ...
    _y_plus(x, y) {
        return Math.abs(this.semiminor_axis *
    ...
    construct_scene() {
        ...
        for (let i = 0; i < this.width - 1; i++) {
            for (let j = 0; j < this.height; j++) {
                this.add(`l(${i},${j})(${i + 1},${j})`, new Line(this.eq_position(i, j), this.eq_position(i + 1, j))
                    .set_stroke_width(0.05)
                    .set_alpha(0.6));
            }
        }
        ...
        for (let i = 0; i < this.width; i++) {
            for (let j = 0; j < this.height - 1; j++) {
                this.add(`l(${i},${j})(${i},${j + 1})`, new Line(this.eq_position(i, j), this.eq_position(i, j + 1))
                    .set_stroke_width(0.05)
                    .set_alpha(0.6));
            }
        }
...
export class WaveSimTwoDimHeatMapScene extends SceneFromSimulator {
    constructor(canvas, imageData, width, height) {
        super(canvas);
        this.add("heatmap", new HeatMap(width, height, -1, 1, new Array(width * height).fill(0)));
...
export class WaveSimTwoDimThreeDScene extends InteractivePlayingThreeDScene {
    ...
    construct_scene() {
        ...
        for (let i = 0; i < this.width - 1; i++) {
            for (let j = 0; j < this.height; j++) {
                this.add(`l(${i},${j})(${i + 1},${j})`, new Line3D(this.eq_position(i, j), this.eq_position(i + 1, j))
                    .set_stroke_width(0.05)
                    .set_alpha(0.3));
            }
        }
        ...
        for (let i = 0; i < this.width; i++) {
            for (let j = 0; j < this.height - 1; j++) {
                this.add(`l(${i},${j})(${i},${j + 1})`, new Line3D(this.eq_position(i, j), this.eq_position(i, j + 1))
                    .set_stroke_width(0.05)
                    .set_alpha(0.3));
            }
        }
    ...
    add_callbacks(i, j, dot) {
        dot.add_callback(() => this.simulator.set_val(this.simulator.index(i + this.width_buffer(), j + this.height_buffer()), dot.get_center()[2]));
        if (i < this.width - 1) {
            dot.add_callback(() => this.get_mobj(`l(${i},${j})(${i + 1},${j})`).move_start(dot.get_center()));
        }
        if (i > 0) {
            dot.add_callback(() => this.get_mobj(`l(${i - 1},${j})(${i},${j})`).move_end(dot.get_center()));
        }
        if (j < this.height - 1) {
            dot.add_callback(() => this.get_mobj(`l(${i},${j})(${i},${j + 1})`).move_start(dot.get_center()));
        }
        if (j > 0) {
            dot.add_callback(() => this.get_mobj(`l(${i},${j - 1})(${i},${j})`).move_end(dot.get_center()));
        }
        dot.add_callback(() => {
            this.draw();
    ...
    update_mobjects() {
        ...
        for (let i = 0; i < this.width; i++) {
            for (let j = 0; j < this.height; j++) {
                new_z = vals[this.simulator.index(i + this.width_buffer(), j + this.height_buffer())];
                dot = this.get_mobj(`p(${i},${j})`);
...
//# sourceMappingURL=wavesim.js.map