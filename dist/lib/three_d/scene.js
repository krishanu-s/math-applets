import { vec3_sum, vec3_sub, matmul_vec, matmul_mat, mat_inv, rot_z_matrix, rot_y_matrix, rot_x_matrix, rot_matrix, rot, rot_x, rot_y, rot_z, } from "./matvec.js";
import { Scene } from "../base";
import { ThreeDLineLikeMObject, ThreeDFillLikeMObject, ThreeDMObjectGroup, } from "./mobjects.js";
...
export class Camera3D {
    ...
    rot_view_z(angle) {
        this.camera_frame_inv = matmul_mat(this.camera_frame_inv, rot_z_matrix(-angle));
    }
    // Rotate the camera matrix around the y-axis.
    rot_view_y(angle) {
        this.camera_frame_inv = matmul_mat(this.camera_frame_inv, rot_y_matrix(-angle));
    }
    // Rotate the camera matrix around the x-axis.
    rot_view_x(angle) {
        this.camera_frame_inv = matmul_mat(this.camera_frame_inv, rot_x_matrix(-angle));
    }
    // Rotate the camera matrix around a given axis
    rot_view(axis, angle) {
        this.camera_frame_inv = matmul_mat(this.camera_frame_inv, rot_matrix(axis, -angle));
    }
