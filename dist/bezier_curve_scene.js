...
var SmoothClosedPathBezierHandleCalculator = class {
  constructor(n) {
    this.n = n;
    let below_diag = (void 0)([n - 1], "float32");
    let diag_list = [3];
    for (let i = 0; i < n - 2; i++) {
      diag_list.push(4);
    }
    ...
    let above_diag = (void 0)([n - 1], "float32");
    this.result = (void 0)([n, n + 1], "float32");
    for (let i = 0; i < n; i++) {
      this.result.set([i, i], 4);
      this.result.set([i, i + 1], 2);
    }
    let v = (void 0)([n], "float32");
    v.set([0], 1);
    v.set([n - 1], 1);
    let q = (void 0)([n], "float32");
    q.set([0], 1);
    q.set([n - 1], 1);
    for (let i = 0; i < n - 1; i++) {
    ...
    for (let i = n - 2; i >= 0; i--) {
      ...
      q.set([i], q.get([i]) - q.get([i + 1]) * scale);
    }
    for (let i = 0; i < n; i++) {
      ...
      q.set([i], q.get([i]) * scale);
    }
    let m = (void 0)(this.n).subtract((void 0)(q, v).multiply(1 / (1 + (void 0)(v, q))));
  ...
  get_bezier_handles(a) {
    ...
    h2.set(
      [this.n - 1, 0],
      2 * a.get([0, 0]) - h1.get([0, 0])
    );
    h2.set(
      [this.n - 1, 1],
      2 * a.get([0, 1]) - h1.get([0, 1])
    );
    return [h1, h2];
  }
};
...
var ClosedCurveScene = class extends Scene {
  constructor(canvas, n) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      a_list.push((void 0)(p));
    }
    a_list.push((void 0)(this.calculate(0)));
    let a = (void 0)(a_list, 0);
  ...
  calculate(t) {
    ...
    for (let w = 0; w < 4; w++) {
      ...
      result[0] = result[0] + this.x_cos_fourier_coeffs[w] * cos - this.x_sin_fourier_coeffs[w] * sin;
      result[1] = result[1] + this.y_cos_fourier_coeffs[w] * cos - this.y_sin_fourier_coeffs[w] * sin;
    }
    return result;
  }
