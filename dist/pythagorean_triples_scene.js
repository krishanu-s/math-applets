...
var GrowLineFromMidpoint = class extends Animation {
  ...
  async _play_frame(scene, i) {
    let line2 = scene.get_mobj(this.mobj_name);
    line2.move_end(
      vec2_sum(
        this.start,
        vec2_scale(
...
var SVGPathMObject = class _SVGPathMObject extends SVGMObject {
  constructor() {
    super(...arguments);
  ...
  from_svg_path(pathElement, scene_scale) {
...
var TeXMObject = class extends SVGPathMObjectGroup {
  async from_latex(latex, scale) {
    ...
    const paths = extractSVGPaths(svgElement).map(
    ...
    for (let i = 1; i < paths.length; i++) {
      mobj.adjoin_to(
        new SVGPathMObject().from_svg_path(paths[i], scale)
      );
    }
...
Parser.prototype.parseString = function(length2) {
...
// src/lib/vectorized/text.ts
var FONT_URLS = {
  // Google Fonts TTF versions
  ROBOTO_TTF: "https://fonts.gstatic.com/s/roboto/v30/KFOmCnqEu92Fr1Mu4mxKKTU1Kvnz.woff",
  // From fonts.google.com, you can download TTF files
  // Or use these CDN URLs for TTF:
  ROBOTO_CDN_TTF: "https://cdn.jsdelivr.net/npm/roboto-font@0.1.0/fonts/roboto/Roboto-Regular.ttf",
  // Open Sans TTF
  OPEN_SANS_TTF: "https://fonts.gstatic.com/s/opensans/v40/memSYaGs126MiZpBA-UvWbX2vVnXBbObj2OVZyOOSr4dVJWUgsjZ0B4gaVQUwaEQbjB_mQ.woff",
  // Arial alternative (public domain font similar to Arial)
  LIBERATION_SANS: "https://cdn.jsdelivr.net/gh/liberationfonts/liberation-fonts@master/liberation-sans/LiberationSans-Regular.ttf",
  // DejaVu Sans (free font)
  DEJAVU_SANS: "https://cdn.jsdelivr.net/npm/dejavu-fonts-ttf@2.37/ttf/DejaVuSans.ttf"
};
async function get_font_from_url(fontUrl) {
  return await load(fontUrl);
}
function renderTextToSVG(text, font) {
...
var TextMObject = class extends SVGPathMObjectGroup {
  async from_text(text, scale, font) {
    this.clear();
  ...
  document.addEventListener("DOMContentLoaded", async function() {
    ...
    await (async function scene_4(width, height) {
      ...
      async function do_scene() {
        ...
        text_mobj.set_center([0, 0]).set_width(3);
        await new Write({ text_mobj }, 40).play(scene);
      }
    })(500, 500);
